...
export const get = (obj, key, def, p, undef) => {
    let keyArr = typeof key === "string" && key.split ? key.split(".") : [key];
...
export const responsive = (styles) => (theme) => {
    ...
    const mediaQueriesList = [
        null,
        ...breakpoints.map((n) => `@media screen and (min-width: ${typeof n === "number" ? n + "px" : n})`),
    ];
    const mediaQueriesObject = Object.fromEntries(Object.entries(theme.breakpoints.values).map(([k, v]) => [
        k,
...
export const sxPropInitialiter = (args) => (theme) => {
    let result = {};
    ...
    for (const key in styles) {
        ...
        if (isObject(val)) {
            if (Object.keys(val).some((a) => Object.keys(theme.breakpoints.values).includes(a))) {
                for (const breakPointType of Object.keys(val)) {
                    ...
                    if (isObject(result[breakPointKey])) {
                        result[breakPointKey] = Object.assign({}, result[breakPointKey], {
                            [transformList[key] || key]: sxPropInitialiter(val)(theme)[breakPointType],
                        });
        ...
        if (multiples[prop]) {
            ...
            if (isObject(value) &&
                Object.keys(value).some((a) => Object.keys(theme.breakpoints.values).includes(a))) {
            }
            else {
                result[prop] =
                    typeof value === "string" && value.includes(".")
...
const sxPropGenerator = (props) => sxPropInitialiter(props.sx)(props.theme);
